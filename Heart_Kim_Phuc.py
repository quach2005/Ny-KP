...
game_form = pygame.font.Font(None,50)
def chu():
    chu_x = game_form.render(f'Kim Phúc Anh Yêu Em',True,(255,105,180))
    chu_y = chu_x.get_rect(center=(400,100))
    screen.blit(chu_x,chu_y)
...
pygame.display.set_caption("Kim Phúc Của Anh")
...
for repeat in range(3):
    for t in np.concatenate((np.linspace(0.18, 3.14-0.18, 1000), np.linspace(3.14+0.18, 2*3.1415-0.18, 1000)), axis=0):
        ...
        # color
        red, white, blue = hsv_to_rgb(0.95, rand(0.2, 0.7), 1)
        alpha = np.random.rand() * 255
        color = (int(red * 255), int(white * 255), int(blue * 255), int(alpha))
